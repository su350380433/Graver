...
#import <Graver/WMGTextAttachment+Event.h>
...
- (WMGBaseCellData *)refreshCellDataWithMetaData:(DemoOrderModel *)item {
    ...
    [imageAttachment addTarget:self action:@selector(buttonAction:) forControlEvents:UIControlEventTouchUpInside];
    ...
    for (DemoOrderButtonInfo *buttonInfo in item.buttonList) {
        ...
        [[buttonAttributedItem.arrayAttachments firstObject] addTarget:self action:@selector(buttonAction:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)buttonAction:(id)sender {
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"高级使用" message:@"高级d点击事件" preferredStyle:UIAlertControllerStyleAlert];
